...
describe('DidDetailsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    jest.spyOn(translateService, 'instant').mockImplementation((key: string | string[]) => {
      if (key === 'app.site.did-details.card.did-information.docs-link.base-registry-doc-url') {
        return mockBaseRegistryDocUrl;
      }
      if (key === 'app.site.did-details.card.did-information.docs-link.api-keys-doc-url') {
  ...
  describe('Error handling', () => {
    it('should show error notification when loading data fails', () => {
