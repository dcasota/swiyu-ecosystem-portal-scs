...
describe('TrustOnboardingWizardComponent', () => {
  ...
  describe('with initialized submission data', () => {
    ...
    describe('updateOrganisationData', () => {
      it('should merge partial data and apply entityEmail business logic', () => {
        service.submission.set(MOCK_SUBMISSION);
        service.submissionRequest = service['mapToSubmissionRequest'](MOCK_SUBMISSION);

        expect(service.submissionRequest?.entityEmail).toBe('contact@test.com');

        const partialUpdate: Partial<TrustOnboardingSubmissionRequest> = {
          entityName: {de: 'New Name'},
          contactPerson: {
            firstName: 'Jane',
            lastName: 'Doe',
            phone: '98765',
            email: 'jane.doe@new.com'
          }
        };

        service.updateOrganisationData(partialUpdate);

        expect(service.submissionRequest).toBeDefined();
        expect(service.submissionRequest?.entityName?.de).toBe('New Name');
        expect(service.submissionRequest?.contactPerson?.email).toBe('jane.doe@new.com');
        expect(service.submissionRequest?.entityEmail).toBe('jane.doe@new.com');
        expect(service.submissionRequest?.partnerId).toBe(MOCK_SUBMISSION.partnerId);
      });
    });
  });
});
