...
import {SWISS_PHONE_PATTERN, SWISS_UID_PATTERN} from '../../../../../core/util/validation-patterns';
...
export class PartnerRegistrationStepProfileCreationComponent extends AbstractOnboardingStepComponent {
  ...
  readonly form = this.fb.group({
    ...
    uid: this.fb.control<string>('', Validators.pattern(SWISS_UID_PATTERN)),
    ...
    phone: this.fb.control<string>('', [Validators.required, Validators.pattern(SWISS_PHONE_PATTERN)]),
    ...
    this.form.valueChanges.subscribe(() => {
      if (this.form.valid) {
        ...
        this.wizardService.updatePartnerDetails({
          ...
          addressRegion: value.region ?? undefined,
  ...
  private translateSetup() {
    ...
    this.translateService.get('eportal_GOV_profile_email_subject');
    this.translateService.get('eportal_GOV_profile_email_body');
  ...
  openGovActivationEmail(): void {
    const subject = encodeURIComponent(this.translateService.instant('eportal_GOV_profile_email_subject'));
    const body = encodeURIComponent(this.translateService.instant('eportal_GOV_profile_email_body'));
    window.open(`mailto:test@swiyu.ch?subject=${subject}&body=${body}`, '_self');
  }
}
