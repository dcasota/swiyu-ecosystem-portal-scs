...
export class PartnerRegistrationWizardService {
  ...
  private createProfile(partnerCreationRequest: PartnerCreationRequest): Observable<PagedModelIdentifierResponse> {
    return this.createPartner(partnerCreationRequest).pipe(
      ...
      switchMap(partner => {
        // JEAP oauth-mock builds JWTs from oauth-mock-data (users / business-partner roles). A brand-new
        // BP is not in the token until that role exists in the mock and the user signs in again — there is
        // no supported SPA call to “patch” the same session with an extra bprole.
        if (!this.appConfigService.tokenRefreshEnabled) {
          const roleValue = `${partner.id}:ti_@businesspartner_#read`;
